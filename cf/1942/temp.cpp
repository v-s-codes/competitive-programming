...
// 	cout<<"\n";
	...
	auto y=curr[0];
		...
		for(int i=1;i<=n;i++){
			...
			if(q[i]!=-1 && !stt.count(i)){
				auto temp=q[i];
				if(y.empty()){q[i]=x[0]; stt.insert(x[0]);
				q[x[0]]=temp; x.pop_back();}
				else{q[i]=y.back(); stt.insert(y.back());
				q[y.back()]=temp; y.pop_back();}
			}
		}
            // for(auto z:q)cout<<z<<" "; cout<<"\n";
			vector<int> p(n+1);
